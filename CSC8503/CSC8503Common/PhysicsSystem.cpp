...
int TotalMark = 0;
int EnemyMark = 10;
bool winandlose = true;
bool lose = true;
bool win = true;
void PhysicsSystem::Update(float dt) {
	Debug::Print("Your Mark", Vector2(90, 80));
	Debug::Print(std::to_string(TotalMark), Vector2(90, 85));
	Debug::Print("EnemyMark", Vector2(90, 90));
	Debug::Print(std::to_string(EnemyMark), Vector2(90, 95));
	if (winandlose == true && lose == false) {
		//Debug::Print("Win!!", Vector2(50, 50));
		Debug::Print(std::to_string(TotalMark), Vector2(50, 55));
	}
	if (winandlose == false && lose == true) {
		//Debug::Print("Lose!!", Vector2(50, 50));
		Debug::Print(std::to_string(TotalMark), Vector2(50, 55));
	}
...
/// <param name="b"></param>
/// <param name="p"></param>
void PhysicsSystem::AccelerationBlock1Object(GameObject& a, GameObject& b, CollisionDetection::ContactPoint& p) const {
	PhysicsObject* physA = a.GetPhysicsObject();
	PhysicsObject* physB = b.GetPhysicsObject();
	...
	if (totalMass == 0) {
		...
		physB->ApplyLinearImpulse(Vector3(-2, 0, -40));

		physB->ApplyAngularImpulse(Vector3(-5, 0, -5));

}

void PhysicsSystem::AccelerationBlock2Object(GameObject& a, GameObject& b, CollisionDetection::ContactPoint& p) const {
	PhysicsObject* physA = a.GetPhysicsObject();
	PhysicsObject* physB = b.GetPhysicsObject();
	...
	if (totalMass == 0) {
		...
		physB->ApplyLinearImpulse(Vector3(-5, 2, -50));

		physB->ApplyAngularImpulse(Vector3(-5, 0, -5));
}

void PhysicsSystem::AccelerationBlock3Object(GameObject& a, GameObject& b, CollisionDetection::ContactPoint& p) const {
	PhysicsObject* physA = a.GetPhysicsObject();
	PhysicsObject* physB = b.GetPhysicsObject();
	...
	if (totalMass == 0) {
		...
		physB->ApplyLinearImpulse(Vector3(-5, 2, 40));

		physB->ApplyAngularImpulse(Vector3(-5, 0, -5));
}

void PhysicsSystem::AccelerationBlock4Object(GameObject& a, GameObject& b, CollisionDetection::ContactPoint& p) const {
	PhysicsObject* physA = a.GetPhysicsObject();
	PhysicsObject* physB = b.GetPhysicsObject();
	...
	if (totalMass == 0) {
		...
		physB->ApplyLinearImpulse(Vector3(0, 3, 0));
		physB->ApplyAngularImpulse(Vector3(-5, 0, -5));

}

void PhysicsSystem::AccelerationBlock5Object(GameObject& a, GameObject& b, CollisionDetection::ContactPoint& p) const {
	PhysicsObject* physA = a.GetPhysicsObject();
	PhysicsObject* physB = b.GetPhysicsObject();
	...
	if (totalMass == 0) {
		...
		physB->ApplyLinearImpulse(Vector3(-10, 2, 50));

		physB->ApplyAngularImpulse(Vector3(-5, 0, -5));
}

void PhysicsSystem::SpinObstacleObject(GameObject& a, GameObject& b, CollisionDetection::ContactPoint& p) const {
	PhysicsObject* physA = a.GetPhysicsObject();
	PhysicsObject* physB = b.GetPhysicsObject();
	...
	physB->ApplyLinearImpulse(Vector3(-4, 0, -10));
	physA->ApplyAngularImpulse(Vector3(0, impulseForce * 2, 0));
	physB->ApplyAngularImpulse(Vector3(-5, 0, -5));
}
/// <summary>
/// daozhe
...
void PhysicsSystem::ImpulseResolveCollision(GameObject& a, GameObject& b, CollisionDetection::ContactPoint& p)const {
	...
	if (a.GetName() == "Block1" && b.GetName() == "sphereplayer") {
		AccelerationBlock1Object(a, b, p);
		return;
	}
	if (a.GetName() == "Block2" && b.GetName() == "sphereplayer") {
		AccelerationBlock2Object(a, b, p);
		return;
	}
	if (a.GetName() == "Block3" && b.GetName() == "sphereplayer") {
		AccelerationBlock3Object(a, b, p);
		return;
	}
	if (a.GetName() == "Block4" && b.GetName() == "sphereplayer") {
		AccelerationBlock4Object(a, b, p);
		return;
	}
	if (a.GetName() == "Block5" && b.GetName() == "sphereplayer") {
		AccelerationBlock5Object(a, b, p);
		return;
	}
	if (a.GetName() == "SpinObstacle" && b.GetName() == "sphereplayer") {
		SpinObstacleObject(a, b, p);
		return;
	}
	...
	if (a.GetName() == "Emery" && b.GetName() == "coin") {
		CoinObject(a, b, 'G', p);
		EnemyMark += 10;
		return;
	}
	//Jiajiajia
	if (a.GetName() == "Win" && b.GetName() == "sphereplayer") {
		winandlose = true;
		lose = false;
		Debug::Print("Win!!", Vector2(50, 50));
	...
	if (a.GetName() == "LoseLand" && b.GetName() == "sphereplayer") {
		winandlose = false;
		lose = true;
		Debug::Print("Lose!!", Vector2(50, 50));
	}

	if (a.GetName() == "Emery" && b.GetName() == "sphereplayer") {
		winandlose = false;
		lose = true;
		Debug::Print("Lose!!", Vector2(50, 50));
	}
...
void PhysicsSystem::CheckWin(GameObject& a, GameObject& b, CollisionDetection::ContactPoint& p) const {
	PhysicsObject* physA = a.GetPhysicsObject();
	PhysicsObject* physB = b.GetPhysicsObject();
	...
	//physB->ApplyLinearImpulse(Vector3(-3, 0, 10));
	//physB->ApplyAngularImpulse(Vector3(-5, 0, -5));
}

void PhysicsSystem::CoinObject(GameObject& a, GameObject& b, char n, CollisionDetection::ContactPoint& p) const {
	PhysicsObject* physA = a.GetPhysicsObject();
	PhysicsObject* physB = b.GetPhysicsObject();
...
void PhysicsSystem::EmenyObject(GameObject& a, GameObject& b, char n, CollisionDetection::ContactPoint& p) const {
	PhysicsObject* physA = a.GetPhysicsObject();
	PhysicsObject* physB = b.GetPhysicsObject();
